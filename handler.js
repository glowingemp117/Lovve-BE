...
const { socketMongoPOST, getSocketUsingSocket_id, deleteAll, getuser_idFromSocket_id, getUserTypeByUserId, sendNotificationUserOffline, getUserByUserId } = require('./backend/controllers/socketController');
app.use(fileUpload({
...
app.use('/conversation', cors(), require('./backend/routes/conversationRoutes'));
...
(async () => {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('message', async (data) => {
      ...
      const messageObj = { sender: new mongoose.Types.ObjectId(data.sender_id), message: data.payload };
      await Conversation.updateOne({ worker_id, employer_id }, { $push: { messages: messageObj } });
